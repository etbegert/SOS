...
public class CPU implements Runnable {
	...
	public void executeInstr(int[] instr) {
		...
		switch (instr[0]) {
			...
			case LOAD:
				...
				setReg(instr[1], m_RAM.read(getBASE() + getReg(instr[2])));
				break;
			case SAVE:
				// Convert from logical to physical memory before save
				boundaryCheck(getBASE() + getReg(instr[2]));
				m_RAM.write(getBASE() + getReg(instr[2]), getReg(instr[1]));
				break;
			case TRAP:
	...
	public void run() {
		...
		while (getPC() < getLIM() && getPC() < getSP()) {
			...
			//if m_ticks is a multiple of the CLOCK_FREQ, generate an interrupt
