...
public class SOS implements CPU.TrapHandler
{
    ...
    private int defragmentAndAllocate(int size)
    {
    	...
    	{
    		pi.move(nextBase);
    		nextBase = pi.getRegisterValue(m_CPU.LIM);
    	}// for
    	m_freeList.add(new MemBlock(nextBase + size, m_RAM.getSize() - nextBase - size));
    	return nextBase;
    ...
    public void createIdleProcess()
    {
        ...
        if(baseAddr == -1)
        {
        	System.out.println("Could not allocate block of size " + progArr.length + ". Exiting.");
        	//TODO: Correct way of exiting?
        	System.exit(1);
        }
    ...
    public void scheduleNewProcess()
    {
        ...
        if (temp != null)
        {
            ...
            m_currProcess = temp;
            SOS.debugPrintln("Moving Process id:"
                    + m_currProcess.getProcessId() + " from Ready to Running.");
        }
        ...
        // If no more processes exist, exit
        debugPrintln("processID:" + m_currProcess.getProcessId());
    ...
    public void createProcess(Program prog, int allocSize)
    {
        ...
        if (program.length > allocSize)
        {
            m_CPU.returnError(CPU.ERROR_LARGE_PROG);

            // Create a system call to end process
            push(SYSCALL_EXIT);
            systemCall();
        }
        int addr = allocBlock(program.length);
        // If allocBlock fails, return to caller
        if(addr == -1)
        {
        	System.out.println("Could not allocate block of size " + program.length);
        	return;
        }
        ...
        {
            m_RAM.write(m_CPU.getBASE() + i, program[i]);

        }

        // setting limit to allocSize above current base
        m_CPU.setLIM(m_CPU.getBASE() + allocSize - 1);
        ...
        SOS.debugPrintln("Adding Process: " + tempProc.getProcessId()
                + " at location:" + m_CPU.getBASE() + " of size:" + allocSize);

        // Add and change to process
    ...
    public void interruptIOWriteComplete(int devID, int addr)
    {
        ...
        // Debug processes
        debugPrintln("Current Process:" + m_currProcess.getProcessId()
                + ". New Process" + pcb.getProcessId());
    ...
    private void syscallOpen()
    {
        ...
        // if the current device already opened it
        // else if (dev.containsProcess(m_currProcess)) {
        // push(ERROR_DEV_IS_OPEN);
        // }
    ...
    private void syscallExec()
    {
        ...
        //TODO:Delete code? See Assignment 7.2.i
    ...
    Comparable<ProcessControlBlock>
    {
        ...
        {
        	if(m_CPU.boundaryCheck(newBase))
        	{
        		int oldBase = this.registers[m_CPU.BASE];
        		int progSize = this.registers[m_CPU.LIM] - this.registers[m_CPU.BASE];
        		int stackSize = this.registers[m_CPU.LIM] - this.registers[m_CPU.SP];
        		//if current process adjust CPU registers
        		if(this == m_currProcess)
        		{
        			int newPC = this.registers[m_CPU.PC] - this.registers[m_CPU.BASE];  
        			m_CPU.setBASE(newBase);
        			m_CPU.setLIM(newBase + progSize);
        			m_CPU.setPC(newBase + newPC);
        			m_CPU.setSP(m_CPU.getLIM() - stackSize);
        		}
        		//Adjust the process' register values
        		this.registers[m_CPU.BASE] = newBase;
        		this.registers[m_CPU.LIM] = newBase + progSize;
        		this.registers[m_CPU.SP] = this.registers[m_CPU.LIM] - stackSize; 
